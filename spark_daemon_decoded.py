...
class SparkDaemonHandler(BaseHTTPRequestHandler):
            ...
            if path == "/api/v1/hosts/maintenance" and payload.get("action") == "leave":
                ...
                try:
                    ...
                    if dfs_engine == "linstor":
                        start_cmd = "systemctl start zookeeper hydra-db aether linstor-controller spectrum bifrost dagur mimir vali catalyst gatoway logos mipha daruk"
                    else:
                        start_cmd = "systemctl start zookeeper hydra-db aether spectrum bifrost dagur mimir vali catalyst gatoway logos mipha daruk"
    ...
    def handle_node_status(self):
        ...
        services = ["zookeeper", "hydra-db", "daruk", "aether", "spark-daemon", "spectrum", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "logos", "mipha", "agahnim", "slate"]
        svc_map = {
            "zookeeper": "ZooKeeper",
            "hydra-db": "HydraDB",
            "daruk": "Daruk",
        ...
        if now - globals()['LAST_PIDS_CACHE_TIME'] > 10 or not globals()['SERVICE_PIDS_CACHE']:
            ...
            # Native services
            native_svcs = ["spark-daemon", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "logos", "mipha", "daruk", "agahnim"]
            if "urbosa" in services:

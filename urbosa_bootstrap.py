...
def main():
    ...
    if rc_t0 == 0:
        ...
        if not routers:
            print("No Tier-0 routers found. Creating default topology (T0 -> T1 -> Segment)...")
            
            t0_id = str(uuid.uuid4())
            t1_id = str(uuid.uuid4())
            seg_id = str(uuid.uuid4())
            
            # Default T0 Gateway (Masquerading enabled)
            cql_t0 = f"""
            INSERT INTO hydra.urbosa_t0_routers (router_id, name, uplink_interface, uplink_ip, gateway_ip, nat_rules)
            VALUES ({t0_id}, 'T0-Default-Edge', 'ens192', '10.10.102.250/24', '10.10.102.1', '{{"snat": true}}');
            """
            run_cql_query(cql_t0)
            
            # Default T1 Router linked to T0
            cql_t1 = f"""
            INSERT INTO hydra.urbosa_t1_routers (router_id, name, t0_link_id, dhcp_enabled)
            VALUES ({t1_id}, 'T1-Default-Router', {t0_id}, true);
            """
            run_cql_query(cql_t1)
            
            # Default Overlay Segment (VNI 10001, Subnet 10.0.1.0/24) linked to T1
            cql_seg = f"""
            INSERT INTO hydra.urbosa_segments (segment_id, name, vni, t1_link_id, subnet_cidr, gateway_ip, dhcp_enabled, dhcp_start, dhcp_end)
            VALUES ({seg_id}, 'Default-Overlay-Segment', 10001, {t1_id}, '10.0.1.0/24', '10.0.1.1', true, '10.0.1.100', '10.0.1.250');
            """
            run_cql_query(cql_seg)

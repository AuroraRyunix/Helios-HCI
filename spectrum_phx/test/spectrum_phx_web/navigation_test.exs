defmodule SpectrumPhxWeb.NavigationTest do
  ...
  describe "the navigation table matches the router" do
    test "every entry is reachable when signed in", %{conn: conn} do
      for {id, label, path} <- Layouts.nav_items() do
    ...
    test "every entry is behind authentication", %{conn: _conn} do
      for {_id, _label, path} <- Layouts.nav_items() do
    ...
    test "every entry appears in the rendered header", %{conn: conn} do
      ...
      for {_id, label, path} <- Layouts.nav_items() do

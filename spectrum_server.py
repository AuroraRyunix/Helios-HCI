...
def init_db():
    ...
    # Detect tier and dfs_engine from local storage-pools.json
    detected_tier = "HDD"
    ...
    try:
        if os.path.exists("/etc/hci/aether/storage-pools.json"):
            with open("/etc/hci/aether/storage-pools.json", "r") as f:
                pdata = json.load(f)
                dfs_engine = pdata.get("dfs_engine", "glusterfs")
    ...
    """
    scrub_cmd = "drbdadm status || true" if dfs_engine == "linstor" else "podman exec systemd-aether gluster volume status || true"
    ...
    for i in range(15):
        ...
        if rc == 0:
            ...
            if (rc2 == 0 and rc3 == 0 and rc4 == 0 and rc5 == 0 and rc6 == 0 and 
                ...
                rc14 == 0 and rc15 == 0 and rc16 == 0 and rc17 == 0 and rc18 == 0):
...
def metrics_and_cluster_monitor_loop():
    ...
    while True:
        try:
            ...
            try:
                # Check dfs_engine from local storage-pools.json
                dfs_engine = "glusterfs"
                try:
                    if os.path.exists("/etc/hci/aether/storage-pools.json"):
                        with open("/etc/hci/aether/storage-pools.json", "r") as f:
                ...
                if dfs_engine == "linstor":
                    ...
                    for cap_ip in candidate_ips:
                        cmd_sp = f"podman exec -e LS_CONTROLLERS={controllers_str} systemd-linstor-controller linstor storage-pool list"
                    ...
                    # GlusterFS path: query each online storage node
                            ...
                            if rc_p == 0:
                                try:
                                    ...
                                    for disk in local_disks:
                                        ...
                                        # Estimate used space via df
                                        rc_df, stdout_df, _ = run_remote_spark(node_ip, f"df -BG {dev} | tail -n 1")
                                        if rc_df == 0 and stdout_df.strip():
                                            parts = stdout_df.strip().split()
                                            if len(parts) >= 3:
                                                try:
                                                    disk_used_gb = int(parts[2].replace("G", ""))
                                                except:
                                                    pass
                ...
                pools.insert(0, {
                    ...
                    "type": "Aether Distributed DRBD" if dfs_engine == "linstor" else "Aether Distributed GlusterFS",
...
class SpectrumHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/auth/check":
            ...
            if any(a.get("type") == "critical" for a in alerts):
                ...
                resilience_ftt = max(0, redundancy_factor - 1)
    ...
    def do_POST(self):
        ...
        if path == "/api/login":
            ...
            # Start catalyst task
            log_catalyst_task("valhalla", "upload_image", "running", 0, {"filename": filename, "size_bytes": content_length}, task_id=task_id, created_at=created_at_ms)
            ...
            try:
                ...
                with open(temp_path, "wb") as f:
                    while bytes_remaining > 0:
                        ...
                        if progress - last_progress >= 5:
                            log_catalyst_task("valhalla", "upload_image", "running", progress, {"filename": filename, "size_bytes": content_length}, task_id=task_id, created_at=created_at_ms)
                            last_progress = progress
                
                mv_cmd = f"mv {temp_path} {target_path} && chmod 666 {target_path}"
                rc, out, err = run_remote_spark("127.0.0.1", mv_cmd)
                if rc != 0:
                    raise Exception(f"Failed to move image to storage: {err or out}")
                ...
                # Background sync to all other hosts
                def bg_sync():
                    dfs_engine = "glusterfs"
                    try:
                        with open("/etc/hci/cluster.json", "r") as f:
                            dfs_engine = json.load(f).get("dfs_engine", "glusterfs")
                    except:
                        pass
                    if dfs_engine != "glusterfs":
                        # Linstor/DRBD mode: copy to the node where default-image-container is mounted
                        mounted_node_ip = None
                        ...
                        for node in nodes_list:
                            ...
                            if nip == local_ip:
                                import subprocess
                                rc_m = subprocess.call(f"mountpoint -q {image_dir}", shell=True)
                                if rc_m == 0:
                                    mounted_node_ip = nip
                                    break
                            else:
                                rc_m, _, _ = run_remote_spark(nip, f"mountpoint -q {image_dir}")
                                if rc_m == 0:
                                    mounted_node_ip = nip
                                    break
                        if mounted_node_ip and mounted_node_ip != local_ip:
                            sync_cmd = f"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@{mounted_node_ip} 'mkdir -p {image_dir}' && scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {target_path} root@{mounted_node_ip}:{target_path} && rm -f {target_path}"
                        ...
                        for line in stdout_n.splitlines():
                            ...
                            if line.startswith("{") and line.endswith("}"):
                                try:
                                    ...
                                    pass
                    local_ip = os.environ.get("LOCAL_HYPERVISOR_IP", "127.0.0.1")
                    other_ips = [n["ip"] for n in nodes if n["ip"] != local_ip]
                    for other_ip in other_ips:
                        sync_cmd = f"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@{other_ip} 'mkdir -p {image_dir}' && scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {target_path} root@{other_ip}:{target_path}"
                        subprocess.Popen(sync_cmd, shell=True)

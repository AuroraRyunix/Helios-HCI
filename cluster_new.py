...
"""
        ...
        if rc == 0 and db_b64:
            ...
            for target_ip in ips[1:]:
                ...
                time.sleep(3)  # Wait for container cleanup and lock release
                ...
                # Verify and wait for the controller to listen on port 3370
                controller_ready = False
                for _ in range(30):
                    rc_check, out_check, _ = run_remote_spark(target_ip, "ss -tlnp | grep 3370")
                    if rc_check == 0 and "3370" in out_check:
                        controller_ready = True
                        break
                    time.sleep(1)
                if not controller_ready:
                    print(f"[ERROR] Linstor Controller failed to start on port 3370 on {target_ip} after database sync.")
                    sys.exit(1)

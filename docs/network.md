...
| **Sidon** (storage replication) | `9105` | TCP, **mutual TLS** | Cluster Mesh | Journal appends, extent-group transfer, epoch fencing and forwarded guest I/O between per-node data-path daemons. One connection per node **pair**, independent of VM count — which was the whole complaint about DRBD's per-device model. Both ends present a certificate from the cluster CA: the port carries `FENCE` as well as guest writes, so an unauthenticated peer could stop a healthy owner's I/O. A one-host cluster binds loopback and skips TLS, since there is nothing to replicate to. See [sidon.md](./sidon.md). |

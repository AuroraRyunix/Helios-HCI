...
```mermaid
graph TD
    DB[(ScyllaDB hydra.gatoway_networks)] -->|Polled every 5s| Flow[Gatoway Daemon]
    Flow -->|Auto-detects| Uplink[Physical Uplink Interface e.g. ens192]
    Flow -->|Creates sub-interfaces| Sub[VLAN Tagged Interface e.g. ens192.100]
    Flow -->|Creates bridge| Bridge[VLAN Bridge e.g. br-vlan-100]

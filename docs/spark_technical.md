...
- Resolves process PIDs (reads systemd `MainPID` properties or fetches PID mappings via `podman top`).
...
- Sets up an HTTPS server (`ThreadingHTTPServer`) on port `9099`.
- Binds SSL context:
...
### API Endpoints
- **`POST /api/v1/execute`**: Extracts a JSON command payload, executes it using `subprocess.Popen` in the host namespace, and returns the exit status code, `stdout`, and `stderr` buffers.
...
- Runs in a separate daemon thread on service startup:
  ...
  5. Starts local cluster database, storage, and logic daemons sequentially.

    ...
    try:
        ...
        for kp in key_paths:
            if os.path.exists(kp):
                try:
                    ssh.connect(ip, username="root", key_filename=kp, timeout=10)
                    connected = True
                    print(f"[Mipha HA] Connected to {ip} via SSH key {kp}")
                    break
                except Exception:
                    pass
        if not connected:
            ssh.connect(ip, username="root", password="ArtPanCooking249!", timeout=10)
            print(f"[Mipha HA] Connected to {ip} via SSH password")
            connected = True
            
        if connected:
            fence_cmd = "systemctl stop libvirtd virtqemud || true; pkill -9 qemu-system-x86_64 || true; pkill -9 qemu || true"
            stdin, stdout, stderr = ssh.exec_command(fence_cmd)
            exit_status = stdout.channel.recv_exit_status()
            print(f"[Mipha HA] SSH Fencing command exit code: {exit_status}")
            ssh.close()
            return True
    except Exception as e:
        print(f"[Mipha HA] SSH Fencing failed for host {ip}: {e}")
    return False
            ...
            for h in target_hosts:
                ...
                if not spark_ok:
                    ...
                    if db_status == "DOWN":
                        ...
                        storage_name = "Linstor/DRBD" if dfs_engine == "linstor" else "GlusterFS"
                        print(f"[Mipha HA] Host {hostname} ({ip}) is back online! Starting rejoining and {storage_name} sync sequence...")
                        ...
                        if dfs_engine == "linstor":
                            print(f"[Mipha HA] Linstor/DRBD storage engine active, skipping GlusterFS self-heal command.")
                        else:

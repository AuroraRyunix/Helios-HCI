    ...
    try {
        ...
        wasmPromise = fetch(wasmUrl)
            ...
            .then(function(results) {
                ...
                console.log("WASM SPICE LZ decompressor loaded successfully.");
            })
            .catch(function(err) {
                console.error("Failed to load WASM SPICE LZ decompressor, falling back to JS:", err);
            });
    } catch (e) {
        console.error("Error setting up WASM SPICE URL, falling back to JS:", e);
    }
    return wasmPromise;
}

// Trigger loading of WASM immediately
loadWasm();
...
{
    if (lz_image.decompressed_image)
    {
        return lz_image.decompressed_image;
    }
    var at;
    if (lz_image.type === Constants.LZ_IMAGE_TYPE_RGB32 || lz_image.type === Constants.LZ_IMAGE_TYPE_RGBA)
    {
        ...
        if (wasmInstance && wasmMemory) {
            ...
            var required_mem = out_ptr + ret.data.length;
            ...
            at = wasmInstance.exports.lz_rgb32_decompress(
                in_ptr,
                0,
                out_ptr,
                ret.data.length,
                lz_image.type === Constants.LZ_IMAGE_TYPE_RGBA ? 1 : 0,
                lz_image.type != Constants.LZ_IMAGE_TYPE_RGBA ? 1 : 0
            );
            
            var wasm_out_buf = new Uint8Array(wasmMemory.buffer, out_ptr, ret.data.length);
            ret.data.set(wasm_out_buf);
        } else {

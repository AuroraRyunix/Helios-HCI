...
**--------------------------------------------------------------------------*/
var _temp_canvas = null;
var _temp_context = null;
var _alpha_canvas = null;
var _alpha_context = null;
...
function putImageDataWithAlpha(context, d, x, y)
{
    if (!_temp_canvas)
    {
        ...
        _temp_context = _temp_canvas.getContext("2d");
    }
    if (_temp_canvas.width < d.width)
        _temp_canvas.width = d.width;
    if (_temp_canvas.height < d.height)
        _temp_canvas.height = d.height;

    _temp_context.putImageData(d, 0, 0, 0, 0, d.width, d.height);
    context.drawImage(_temp_canvas, 0, 0, d.width, d.height, x, y, d.width, d.height);
}
...
{
    var u32 = new Uint32Array(d.data.buffer, d.data.byteOffset, d.data.byteLength / 4);
    var len = u32.length;
    for (var i = 0; i < len; i++)
        u32[i] |= 0xff000000;
}
...
function handle_draw_jpeg_onload()
{
    ...
    **----------------------------------------------------------*/
    ...
    if (this.alpha_img)
    {
        if (!_alpha_canvas)
        {
            _alpha_canvas = document.createElement("canvas");
            _alpha_context = _alpha_canvas.getContext("2d");
        }
        var width = this.alpha_img.width;
        var height = this.alpha_img.height;
        if (_alpha_canvas.width < width)
            _alpha_canvas.width = width;
        if (_alpha_canvas.height < height)
            _alpha_canvas.height = height;

        _alpha_context.globalCompositeOperation = 'source-over';
        _alpha_context.clearRect(0, 0, width, height);
        _alpha_context.putImageData(this.alpha_img, 0, 0, 0, 0, width, height);
        _alpha_context.globalCompositeOperation = 'source-in';
        _alpha_context.drawImage(this, 0, 0, width, height, 0, 0, width, height);

        context.drawImage(_alpha_canvas, 0, 0, width, height, this.o.base.box.left, this.o.base.box.top, width, height);

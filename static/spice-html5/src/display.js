...
import { WebGLContextEmulator } from './webgl_renderer.js';
...
function SpiceDisplayConn()
{
    ...
    this.worker_msg_id = 0;
    
    try {
        var workerUrl = new URL('./lz_worker.js', import.meta.url).href;
        this.worker = new Worker(workerUrl);
        var wasmUrl = new URL('../../vendor/wasm-spice/wasm_spice.wasm', import.meta.url).href;
        this.worker.postMessage({ type: 'init', wasmUrl: wasmUrl });
        
        var self = this;
        this.worker.onmessage = function(e) {
            if (e.data.type === 'decompressed') {
                var resolve = self.worker_promises[e.data.id];
                if (resolve) {
                    delete self.worker_promises[e.data.id];
                    resolve(e.data);
                }
            }
        };
    } catch (e) {
        console.error("Failed to initialize Web Worker for display channel:", e);
        this.worker = null;
    }
}
...
SpiceDisplayConn.prototype.process_channel_message = function(msg)
{
    ...
    {
        var draw_copy = msg.parsed_message || new Messages.SpiceMsgDisplayDrawCopy(msg.data);
    ...
    if (msg.type == Constants.SPICE_MSG_DISPLAY_SURFACE_CREATE)
    {
        ...
        try {
            canvas.context = new WebGLContextEmulator(canvas);
        } catch (e) {
            console.warn("WebGL Context Emulator failed, falling back to 2D context:", e);
...
SpiceDisplayConn.prototype.hook_events = function()
{
    if (this.primary_surface !== undefined)
    {
        ...
        if (Inputs && Inputs.handle_blur) {
            canvas.addEventListener('blur', Inputs.handle_blur);
        }
...
SpiceDisplayConn.prototype.unhook_events = function()
{
    if (this.primary_surface !== undefined)
    {
        ...
        if (Inputs && Inputs.handle_blur) {
            canvas.removeEventListener('blur', Inputs.handle_blur);
        }
...
SpiceDisplayConn.prototype.original_process_channel_message = SpiceDisplayConn.prototype.process_channel_message;
...
SpiceDisplayConn.prototype.decompress_lz_async = function(lz_image) {
    var self = this;
    return new Promise(function(resolve) {
        ...
        var msgId = ++self.worker_msg_id;
        self.worker_promises[msgId] = resolve;
...
SpiceDisplayConn.prototype.process_queue = function()
{
    if (this.queue_processing || this.message_queue.length === 0)
        return;
    ...
    function next() {
        ...
        var msg = self.message_queue.shift();
        
        if (msg.type == Constants.SPICE_MSG_DISPLAY_DRAW_COPY && self.worker) {
            var draw_copy = new Messages.SpiceMsgDisplayDrawCopy(msg.data);
            msg.parsed_message = draw_copy;
            
            var promises = [];
            
            if (draw_copy.data && draw_copy.data.src_bitmap) {
                var src_bitmap = draw_copy.data.src_bitmap;
                if (src_bitmap.descriptor.type == Constants.SPICE_IMAGE_TYPE_LZ_RGB && src_bitmap.lz_rgb) {
                    var lz_image = src_bitmap.lz_rgb;
                    promises.push(self.decompress_lz_async(lz_image).then(function(result) {
                        if (result) {
                            try {
                                var imgData = new ImageData(new Uint8ClampedArray(result.data), result.width, result.height);
                                lz_image.decompressed_image = imgData;
                            } catch (e) {
                                console.error("Error creating ImageData from worker output:", e);
                            }
                        }
                    }));
                } else if (src_bitmap.descriptor.type == Constants.SPICE_IMAGE_TYPE_JPEG_ALPHA && src_bitmap.jpeg_alpha && src_bitmap.jpeg_alpha.alpha) {
                    var lz_image = src_bitmap.jpeg_alpha.alpha;
                    promises.push(self.decompress_lz_async(lz_image).then(function(result) {
                        if (result) {
                            try {
                                var imgData = new ImageData(new Uint8ClampedArray(result.data), result.width, result.height);
            ...
            if (promises.length > 0) {
                Promise.all(promises).then(function() {
                    try {
                        self.original_process_channel_message(msg);
                    } catch (e) {
                        console.error("Error processing display message after worker decompression:", e);
                    }
                    setTimeout(next, 0);
                }).catch(function(err) {
                    console.error("Error during background decompression:", err);
                    try {
                        self.original_process_channel_message(msg);
                    } catch (e) {
                        console.error("Error processing display message:", e);
                    }
                    setTimeout(next, 0);
                });
                return;

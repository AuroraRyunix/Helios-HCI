...
SpiceLinkAuthTicket.prototype =
{
    to_buffer: function(a, at)
    {
        ...
        var len = this.encrypted_data ? this.encrypted_data.length : (Constants.SPICE_TICKET_KEY_PAIR_LENGTH / 8);
        for (i = 0; i < len; i++)
        {
...
function SpiceMsgcMousePosition(sc, e)
{
    ...
    if (e)
    {
        var canvas = document.getElementById("spice_surface_0") || e.target;
        var rect = canvas.getBoundingClientRect();
        var localX = e.clientX - rect.left;
        var localY = e.clientY - rect.top;

        // Account for CSS 'object-fit: contain' rendering size inside the element bounding box
        var r_canvas = canvas.width / canvas.height;
        var r_box = rect.width / rect.height;
        var w_rendered, h_rendered, x_offset, y_offset;

        if (r_box > r_canvas)
        {
            // Pillarboxed (black bars left and right)
            h_rendered = rect.height;
            w_rendered = rect.height * r_canvas;
            x_offset = (rect.width - w_rendered) / 2;
            y_offset = 0;
        }
        else
        {
            // Letterboxed (black bars top and bottom)
            w_rendered = rect.width;
            h_rendered = rect.width / r_canvas;
            x_offset = 0;
            y_offset = (rect.height - h_rendered) / 2;
        }

        var x_relative = localX - x_offset;
        var y_relative = localY - y_offset;

        var scaleX = w_rendered ? (canvas.width / w_rendered) : 1;
        var scaleY = h_rendered ? (canvas.height / h_rendered) : 1;

        this.x = Math.round(x_relative * scaleX);
        this.y = Math.round(y_relative * scaleY);

...
export default class Display {
    ...
    imageRect(x, y, width, height, mime, arr) {
        ...
        // Threshold-based decode: off-thread decode only for large images (e.g. >= 128x128)
        // to avoid high promise resolution scheduling overhead for many tiny tiles.
        if (typeof window.createImageBitmap === 'function' && (width * height >= 16384)) {
            const start = performance.now();
            let bitmapReady = false;
            let bitmapImage = null;

            const promise = window.createImageBitmap(blob).then((bitmap) => {
                const decodeTime = performance.now() - start;
                if (Math.random() < 0.05) {
                    console.log(`[noVNC Display] Off-thread ImageBitmap decode took ${decodeTime.toFixed(2)} ms`);
                }
                bitmapReady = true;
                bitmapImage = bitmap;
                this._scanRenderQ();
            }).catch((err) => {
                Log.Error("Failed to create ImageBitmap: " + err);
                bitmapReady = true;
            });
        ...
        // Fast-path for small images: use URL.createObjectURL(blob) instead of slow Base64 encoding
        const start = performance.now();
        const img = new Image();
        ...
        img.addEventListener('load', () => {
            ...
            if (Math.random() < 0.05) {
                console.log(`[noVNC Display] Fast-path Image load took ${loadTime.toFixed(2)} ms`);
            }
        });
        img.src = url;
    ...
    _scanRenderQ() {
        ...
        while (ready && this._renderQ.length > 0) {
            ...
            switch (a.type) {
                ...
                case 'bitmap':
                    if (a.getReady()) {
                        const img = a.getImage();
                        if (img) {
                            this.drawImage(img, a.x, a.y);
                            img.close();
                        }
                    } else {
                        ready = false;

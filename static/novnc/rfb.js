...
import Display from "./display.js?v=1.0.18";
...
import H264Decoder from "./decoders/h264.js?v=1.0.18";
...
export default class RFB extends EventTargetMixin {
    ...
    _requestRemoteResize() {
        ...
        if (this._pendingRemoteResize) {
            // Safety timeout: if it has been more than 1000ms since the last resize request,
            // assume the server failed to respond or the update was lost, and allow a new resize.
            if (Date.now() - this._lastResize > 1000) {
                this._pendingRemoteResize = false;
            } else {
                // If a resize is pending, schedule a retry after 100ms so the latest size is eventually sent
                clearTimeout(this._resizeTimeout);
                this._resizeTimeout = setTimeout(this._requestRemoteResize.bind(this), 100);
                return;
            }
        }
        ...
        const size = this._screenSize();
        let width = Math.floor(size.w);
        let height = Math.floor(size.h);

        console.log("[DEBUG RESIZE] Viewport size:", width, "x", height, "Current FB size:", this._fbWidth, "x", this._fbHeight);

        // Guard: skip if container is hidden or not yet laid out (display:none returns 0x0)
        if (width < 100 || height < 100) {
            console.log("[DEBUG RESIZE] Container too small or hidden, skipping resize.");
            return;
        }

        // Do we actually change anything?
        if (width === this._fbWidth && height === this._fbHeight) {
            console.log("[DEBUG RESIZE] Resolution already matches framebuffer, ignoring request.");
            return;
        }

        this._pendingRemoteResize = true;
        this._lastResize = Date.now();
        console.log("[DEBUG RESIZE] Sending setDesktopSize request:", width, "x", height, "screenID:", this._screenID, "flags:", this._screenFlags);
    ...
    _sendEncodings() {
        ...
        if (this._fbDepth == 24) {
            ...
            // ZRLE is fully synchronous in the client and avoids async rendering bottlenecks.
            // We omit Tight/TightPNG/JPEG so that the server is forced to choose ZRLE.
    ...
    _normalMsg() {
        ...
        switch (msgType) {
            case 0:  // FramebufferUpdate
                ...
                if (ret) {
                    this.dispatchEvent(new CustomEvent("fbucomplete"));
    ...
    _handleExtendedDesktopSize() {
        ...
        if (this._FBU.y === 4) {
            this._pendingRemoteResize = true; // Still waiting for guest OS to apply size
        } else {
            this._pendingRemoteResize = false;
        }

        // We need to handle errors when we requested the resize.
        if (this._FBU.x === 1 && this._FBU.y !== 0 && this._FBU.y !== 4) {
            console.log("[DEBUG RESIZE] Server FBU status:", this._FBU.x, "y (status code):", this._FBU.y, "w:", this._FBU.width, "h:", this._FBU.height);
            let msg = "";
            ...
            Log.Warn("Server did not accept the resize request: "
                     + msg);
        } else if (this._FBU.x === 1 && this._FBU.y === 4) {
            console.log("[DEBUG RESIZE] Server forwarded resize request to guest OS (status code 4). Waiting for guest to apply size...");
        } else {
            console.log("[DEBUG RESIZE] Server accepted resize! New size:", this._FBU.width, "x", this._FBU.height);
            this._resize(this._FBU.width, this._FBU.height);
        }
        ...
        if (this._FBU.y === 0) {
            this._requestRemoteResize();
        } else if (this._FBU.y === 4) {
            // Forwarded to guest, wait for guest-initiated resize FBU (x=0 or y=0)
            console.log("[DEBUG RESIZE] Guest resize is pending, skipping sync check.");
        } else {
            // If the server rejected (1, 2, 3), don't loop. Retry once after 2000ms if needed.

...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (dnsForm) {
        dnsForm.addEventListener('submit', (e) => {
            ...
            const payload = {
                ...
                dns_mtu: document.getElementById('dns-mtu-input').value,
    ...
    */
        ...
        if (editDiskListContainer) {
            ...
            if (vm.disks_list && vm.disks_list !== "NONE") {
                ...
                disksPayload.forEach(d => {
                    ...
                    addEditDiskRow(sizeVal, unitVal, containerVal);
                });
            } else {
                addEditDiskRow("10", "GB", "default-vm-container");
            }
        }
    ...
    function saveVmEdit(e) {
        ...
        const nicSelects = editNicListContainer ? editNicListContainer.querySelectorAll('.nic-network-select') : [];
        const networkIds = [];
        nicSelects.forEach(sel => {
            if (sel.value) networkIds.push(sel.value);
        });
    ...
    function addDiskRowToContainer(containerEl, sizeValue = "20", unitValue = "GB", containerValue = "default-vm-container") {
        if (!containerEl) return;
        const row = document.createElement('div');
        row.className = 'disk-row glass-card';
        row.style.display = 'flex';
        row.style.flexDirection = 'column';
        row.style.padding = '12px';
        row.style.borderRadius = '8px';
        ...
        const containers = state.storageContainers && state.storageContainers.length > 0
            ? state.storageContainers
        ...
        row.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span class="disk-title" style="font-family: 'Space Grotesk', sans-serif; font-size: 13px; font-weight: 600; color: #fff;">Disk</span>
                <button type="button" class="btn-remove-disk" style="padding: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; background: none; border: none; color: #ef4444; font-size: 12px; font-family: 'Space Grotesk', sans-serif; font-weight: 600; gap: 4px;">
                    <svg viewBox="0 0 24 24" style="width: 14px; height: 14px; fill: none; stroke: currentColor; stroke-width: 2;"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                    Remove
                </button>
            </div>
            <div style="display: flex; gap: 8px; align-items: center; width: 100%;">
                <div style="flex: 1; display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; color: var(--text-muted); font-family: 'Space Grotesk', sans-serif;">Size</label>
                    <div style="display: flex; gap: 4px;">
                        <input type="number" class="form-input disk-size-val" min="1" value="${sizeValue}" style="flex: 1; height: 32px; font-size: 12px; min-width: 0;" required>
                        <select class="form-input disk-size-unit" style="width: 70px; height: 32px; font-size: 12px; min-width: 70px;">
                ...
                <div style="flex: 1.2; display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; color: var(--text-muted); font-family: 'Space Grotesk', sans-serif;">Storage Container</label>
                    <select class="form-input disk-container-val" style="width: 100%; height: 32px; font-size: 12px;">
                        ${containerOptions}
                    </select>
                </div>
            </div>
    ...
    function addDiskRow(sizeValue = "20", unitValue = "GB", containerValue = "default-vm-container") {
        addDiskRowToContainer(diskListContainer, sizeValue, unitValue, containerValue);
    }

    function addEditDiskRow(sizeValue = "20", unitValue = "GB", containerValue = "default-vm-container") {
        addDiskRowToContainer(document.getElementById('edit-disk-list-container'), sizeValue, unitValue, containerValue);
    }
    ...
    function addNicRow(networkId = '') {
        ...
        if (state.networks && state.networks.length > 0) {
            ...
            selectOptions = `<option value="7a68e0d6-11f8-4e89-9430-b3b44b8bc438">Physical-Direct (System)</option>`;
        }
    ...
    function addEditNicRow(networkId = '') {
    ...
    function runVncHandshake(vmName, isRetry = false) {
        ...
        .then(module => {
            ...
            const wsUrl = `${protocol}//${window.location.host}/api/vms/console/ws?name=${vmName}&token=${getStoredToken()}`;
        ...
        .catch(err => {
            ...
            showVncDisconnected("Library Load Error", "Could not load local noVNC dependencies.");
        });
    }
        ...
        filteredVms.forEach(vm => {
            ...
            if (isRunning) {
                actionBtn = `
                    ...
                    <button class="table-action-btn spice-btn" data-name="${vm.name}" style="margin-right:5px; border-color:#8e44ad; color:#8e44ad;">
                        <svg viewBox="0 0 24 24" class="action-icon" style="fill:#8e44ad;"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H8V4h12v12z"/></svg> SPICE
                    </button>
    ...
    function bindRowButtons(row, vmName) {
        ...
        const spiceBtn = row.querySelector('.spice-btn');
        if (spiceBtn) {
            spiceBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                const host = window.location.hostname;
                const port = window.location.port || (window.location.protocol === 'https:' ? '443' : '80');
                const path = encodeURIComponent(`api/vms/console/ws?name=${vmName}&type=spice&token=${getStoredToken()}`);
                const url = `spice-html5/spice_auto.html?host=${host}&port=${port}&path=${path}&v=1.0.3`;
    ...
    themeCards.forEach(card => {
        ...
        card.addEventListener('click', () => {
            ...
            document.documentElement.classList.remove('theme-sapphire', 'theme-emerald', 'theme-ruby', 'theme-amethyst', 'theme-light', 'theme-grey', 'theme-sunset', 'theme-ocean', 'theme-midnight', 'theme-forest');
    ...
    function getCheckCategory(checkName) {
        const storageChecks = [
            'brick_mounts', 'brick_space_utilization', 'gluster_volume', 
            'glusterfs_heal_pending', 'glusterfs_split_brain', 'storage_capacity', 
    ...
    function getFriendlyCheckName(checkName) {
        const friendly = {
            'glusterfs_heal_pending': 'GlusterFS Heal Pending',
            'glusterfs_split_brain': 'GlusterFS Split Brain',
                        ...
                        <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 12px;">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="t0-uplink-interface" required value="ens192" class="form-input" style="width: 100%; box-sizing: border-box;">
                    ...
                    <form id="edit-t0-form" style="display: flex; flex-direction: column; gap: 15px;">
                        <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 12px;">
                                ...
                                <div class="form-group">
                                    <label style="font-family: 'Space Grotesk', sans-serif; font-size: 13px; font-weight: 600; color: var(--text-primary); display: block; margin-bottom: 5px;">Uplink Interface <span class="required" style="color: var(--color-primary);">*</span></label>
                                    <input type="text" id="edit-t0-uplink-interface" required value="${uplinkInterface}" class="form-input" style="width: 100%; box-sizing: border-box;">
                                </div>
                            </div>
                            <!-- Column 2: IP Configuration & NAT -->
                            <div style="display: flex; flex-direction: column; gap: 12px; border-left: 1px solid var(--border-color); padding-left: 20px;">
    ...
    function renderUrbosaTopology() {
        try {
                ...
                connectedVmNodes.forEach((cvm, idx) => {
                    ...
                    nodes.push({
                        ...
                        details: {
                            ...
                            vcpu: cvm.vm.vcpu || 'N/A',
                            ram: cvm.vm.ram_mb ? `${cvm.vm.ram_mb} MB` : 'N/A',
                            node: cvm.vm.node || 'N/A',
                            template: cvm.vm.os_template || 'N/A',
                            ip: cvm.vm.ip || 'DHCP leased',
                            mac: cvm.vm.mac || 'Auto-generated'
                        }
                    });
            ...
            vmNodes.forEach(node => {
                ...
                const segId = parts[parts.length - 1];
            ...
            links.forEach(link => {
                ...
                let strokeColor = 'rgba(255,255,255,0.15)';
                let flowColor = 'rgba(255,255,255,0.3)';
            ...
            nodes.forEach(node => {
                ...
                outerCircle.setAttribute('fill', '#090b14');
    ...
    function showNodeDetails(node) {
        ...
        let html = `
            <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--border-color); border-radius: 8px; padding: 15px; display: flex; flex-direction: column; gap: 10px;">
                <div style="display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 10px;">
                    <span style="font-size: 20px;">
                        ${node.type === 'uplink' ? '🌐' : node.type === 't0' ? '🛡️' : node.type === 't1' ? '⚡' : node.type === 'segment' ? '☁️' : '💻'}
                    </span>
                    <div>
                        <h4 style="margin: 0; font-family: 'Space Grotesk', sans-serif; font-size: 14px; font-weight: 600; color: #fff;">${node.details.name}</h4>
        ...
        if (node.type === 'uplink') {
            ...
            html += `
                ...
                <div class="inspector-prop"><span>Transit Loopback</span><span>100.64.0.0/16 scope</span></div>
            `;
        } else if (node.type === 'segment') {
            html += `
            ...
            html += `
                ...
                <div class="inspector-prop"><span>MAC Address</span><span>${node.details.mac}</span></div>
                <div class="inspector-prop"><span>CPU Cores</span><span>${node.details.vcpu} vCPUs</span></div>
                <div class="inspector-prop"><span>Memory Limit</span><span>${node.details.ram}</span></div>
                <div class="inspector-prop"><span>OS Template</span><span>${node.details.template}</span></div>
            `;
        }

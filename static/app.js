    ...
    function bindRowButtons(row, vmName) {
        ...
        if (spiceBtn) {
            spiceBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                fetch(`/api/vms/console/token?name=${vmName}&type=spice&token=${getStoredToken()}`)
                .then(res => {
                    if (!res.ok) throw new Error("SPICE Token fetch failed: " + res.statusText);
                    return res.json();
                })
                .then(data => {
                    const targetHost = window.location.hostname;
                    const targetPort = window.location.port || (window.location.protocol === 'https:' ? '443' : '80');
                    const targetPath = encodeURIComponent(`api/vms/console/ws?name=${vmName}&type=spice&token=${data.token}`);
                    const url = `spice-html5/spice_auto.html?host=${targetHost}&port=${targetPort}&path=${targetPath}&name=${vmName}&v=1.0.4`;
                    window.open(url, '_blank');
                })
                .catch(err => {
                    console.error("SPICE token fetch failed:", err);
                    alert("Could not fetch target host console coordinates: " + err.message);
                });
            });
        }

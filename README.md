...
| [Spectrum (Phoenix)](./docs/spectrum_phx.md) | **Prism** | Podman + Elixir/Phoenix LiveView | The console rewrite, running beside the Python tier on port 8444. **It now serves the live console** for the pages that have been rebuilt -- Slate routes those to it and everything else (the remaining six pages, the guest console and the whole HTTP API) to the Python tier, with one sign-in covering both. Renders server-side and never touches the data path — its storage page reads Sidon through spark-daemon, and its image upload streams bytes to the host rather than opening a vdisk. Shipped by `deploy_updates.py`. |
...
* **Active WebUI VIP Failover**: The VIP manager (`bifrost`) evaluates active candidates on port `443` (Slate Ingress) and enforces a local health guard. A node will only bind the VIP if it is the leader AND its local Slate ingress (`443`) and **both** console backends (`8443` and `8444`) are listening. All three are required because `slate_config/dynamic.yml` now points Slate at two backends — the rebuilt pages to `8444`, everything else to `8443` — and a node with `443` up but either backend down returns `502` for whatever that backend serves, which is no better for clients than the blackhole the guard exists to prevent. Failing the guard does not make the console unreachable: every node serves `443` on its own address, so an operator can still reach one directly while the VIP looks for a node with both. If either layer is down or bootstrapping, the VIP floats to a healthy node, and the daemon logs which layer failed. Addresses are compared exactly rather than by substring, so a VIP of `10.10.102.13` is not confused with a host address of `10.10.102.130`.

...
def generate_vm_xml(name, memory, vcpu, firmware, disks_list, iso, boot_device="", host_ip="127.0.0.1", network_id=None):
    ...
    if disks_list and disks_list != "NONE":
        for idx, entry in enumerate(disks_list.split(",")):
            container = entry.split(":")[1] if ":" in entry else "default-vm-container"
            d_path = f"/var/lib/hci/aether/volumes/{container}/{name}.raw" if idx == 0 else f"/var/lib/hci/aether/volumes/{container}/{name}_disk{idx}.raw"
    ...
    if iso:
        for idx, spec in enumerate(iso.split(",")):
            if spec.strip() and spec.strip() != "__empty__":
                ...
                iso_path = f"/var/lib/hci/aether/volumes/default-image-container/{spec.strip()}"
                disk_devices_xml += f"""
  ...
  </cpu>""" if has_kvm else f"""<cpu mode='custom' match='exact'>
    <model>Haswell</model>
    <topology sockets='1' dies='1' clusters='1' cores='{vcpu}' threads='1'/>
  </cpu>"""
    ...
    # Check if network_id is a JSON list of network IDs
    network_ids = []
    if network_id:
        ...
        # Default fallback to direct network if no network specified
        network_ids = ["7a68e0d6-11f8-4e89-9430-b3b44b8bc438"]

    # Generate XML for each interface
    for idx, net_id in enumerate(network_ids):
        # Auto-generate a unique MAC address based on VM name and interface index
        # MAC format: 52:54:00:xx:xx:xx
        import hashlib
...
def select_best_start_host(memory_needed):
    ...
    # Check if a specific host has the default-vm-container mounted
    mounted_host = None
    for h in hosts:
        ip = h["ip"]
        try:
            rc, _, _ = run_remote_spark(ip, "mountpoint -q /var/lib/hci/aether/volumes/default-vm-container")
            if rc == 0:
                mounted_host = ip
                break
        except Exception:
    ...
    if mounted_host:
        sys.stdout.write(f"[Vali Scheduler] Restricting scheduling to host {mounted_host} where default-vm-container is mounted.\n")
        hosts = [h for h in hosts if h["ip"] == mounted_host]
...
def get_linstor_pending_sync():
    ...
    cmd = f"podman exec -e LS_CONTROLLERS={controllers_str} systemd-linstor-controller linstor volume list"
...
def process_queue_task(task):
    ...
    try:
        if action == "start":
            ...
            firmware = vm_data.get("firmware", "bios")
            ...
            # 2. Compile XML
            vm_xml = generate_vm_xml(vm_name, memory, vcpu, firmware, disks_list, iso, boot_device, host_ip=selected_host, network_id=vm_data.get("network_id"))
            import base64
            b64_xml = base64.b64encode(vm_xml.encode("utf-8")).decode("utf-8")
            
            # 3. Define and start VM via host spark-daemon
            cmd = f"virsh -c qemu:///system undefine {vm_name} --keep-nvram || true; echo {b64_xml} | base64 -d > /tmp/{vm_name}.xml && virsh -c qemu:///system define /tmp/{vm_name}.xml && rm /tmp/{vm_name}.xml && virsh -c qemu:///system start {vm_name}"
            ...
            # Run destroy/undefine
            cmd = f"virsh -c qemu:///system destroy {vm_name} || true && virsh -c qemu:///system undefine {vm_name} --keep-nvram || true"
            ...
            # Query current host_ip
            rc_vm, stdout_vm, _ = run_cql_query(f"SELECT JSON host_ip, state FROM hydra.vms WHERE name = '{vm_name}';")
            src_host = ""
            state = ""
            if rc_vm == 0:
            ...
            # Pre-clean stale definition on target host
            run_remote_spark(target_ip, f"virsh -c qemu:///system undefine {vm_name} --keep-nvram || true")
            
            # Live migrate command
            cmd = f"virsh -c qemu:///system migrate --live --persistent --undefinesource --unsafe {vm_name} qemu+ssh://root@{target_ip}/system"
            ...
            if success:
                ...
                stop_cmd = "sleep 2 && systemctl stop spectrum catalyst bifrost dagur mimir vali aether linstor-controller hydra-db gatoway urbosa logos mipha"
            ...
            start_cmd = "systemctl start zookeeper hydra-db aether linstor-controller spectrum bifrost dagur mimir vali catalyst gatoway urbosa logos mipha"
            ...
            if dfs_engine == "linstor":
                ...
                # Trigger GlusterFS self-heal
                print(f"[Maintenance Catalyst Task] Triggering GlusterFS self-heal for default volumes...")
            ...
            for iteration in range(60):
                ...
                if dfs_engine == "linstor":
                    ...
                    print(f"[Maintenance Catalyst Task] GlusterFS sync status - VM entries: {vm_entries}, Image entries: {img_entries}")
                    if vm_entries == 0 and img_entries == 0:
                        synced = True
                        print(f"[Maintenance Catalyst Task] GlusterFS volumes fully synced on host {hostname}!")
                        break
            ...
            if synced:
                ...
                err_msg = "DRBD/GlusterFS volume sync timed out or failed to complete self-heal."
...
def evacuate_host_thread(hostname, target_ip, force_stop):
    try:
        ...
        if not running_vms:
            ...
            # Write state file and stop all cluster services on the target host (except spark-daemon)
            run_remote_spark(target_ip, "mkdir -p /etc/hci && touch /etc/hci/maintenance.state")
            stop_cmd = "systemctl stop spectrum catalyst bifrost dagur mimir vali aether linstor-controller hydra-db gatoway urbosa logos mipha"
            run_remote_spark(target_ip, f"({stop_cmd}) >/dev/null 2>&1 < /dev/null &")
            return

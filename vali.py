    ...
    try:
        if action == "start":
            ...
            if success:
                ...
                stop_cmd = "sleep 2 && systemctl stop spectrum catalyst bifrost dagur mimir vali aether hydra-db zookeeper gatoway"
            ...
            # Set status to NORMAL in ScyllaDB
            cql_up = f"UPDATE hydra.nodes SET status = 'NORMAL', maintenance_mode = false WHERE hostname = '{hostname}';"
            ...
            start_cmd = "systemctl start zookeeper && sleep 3 && systemctl start hydra-db aether spectrum bifrost dagur mimir vali catalyst gatoway"
            run_remote_spark(target_ip, f"({start_cmd}) >/dev/null 2>&1 < /dev/null &")
            
            # Wait for services on the leaving node to bootstrap and stabilize
            print(f"[Maintenance Catalyst Task] Waiting 15 seconds for services on host {hostname} to stabilize...")
            time.sleep(15)
...
def evacuate_host_thread(hostname, target_ip, force_stop):
    try:
        ...
        if not running_vms:
            ...
            # Write state file and stop all cluster services on the target host (except spark-daemon)
            run_remote_spark(target_ip, "mkdir -p /etc/hci && touch /etc/hci/maintenance.state")
            stop_cmd = "systemctl stop spectrum catalyst bifrost dagur mimir vali aether hydra-db zookeeper gatoway"
            run_remote_spark(target_ip, f"({stop_cmd}) >/dev/null 2>&1 < /dev/null &")
            return

...
def generate_vm_xml(name, memory, vcpu, firmware, disks_list, iso, boot_device="", host_ip="127.0.0.1", network_id=None, cpu_model=None, audio_enabled=False):
    ...
    if disks_list and disks_list != "NONE":
        ...
        disk_paths_with_bus = [(f"/var/lib/hci/aether/volumes/default-vm-container/{name}.raw", "virtio")]
...
def process_queue_task(task):
    ...
    try:
        if action == "start":
            ...
            # Storage mount check on target host
            disks_list = vm_data.get("disks_list", "")
            containers_to_check = set()
            if disks_list and disks_list != "NONE":
                for entry in disks_list.split(","):
                    parts = entry.split(":")
                    if len(parts) > 1:
                        containers_to_check.add(parts[1])
                    else:
                        containers_to_check.add("default-vm-container")
            else:
                containers_to_check.add("default-vm-container")
            
            for container in containers_to_check:
                rc_m, _, _ = run_remote_spark(target_ip, f"mountpoint -q /var/lib/hci/aether/volumes/{container}")
                if rc_m != 0:
                    return False, f"Storage container '{container}' is not mounted on target host {target_host}."

...
def show_status_json():
    services = ["zookeeper", "hydra-db", "aether", "spark-daemon", "spectrum", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "logos", "mipha", "daruk"]
    svc_map = {
        "zookeeper": "ZooKeeper",
        "hydra-db": "HydraDB",
        "aether": "Aether",
    ...
    for svc in services:
        ...
        if is_active:
            if svc == "zookeeper":
                ...
                if get_dfs_engine() == "linstor":
                    healthy = check_tcp_port(3366, ip_addr)
                else:
                    vm_mounted = check_mount("/var/lib/hci/aether/volumes/default-vm-container")
                    # Check default-image-container too if it exists on host
                    img_path = "/var/lib/hci/aether/volumes/default-image-container"
                    img_mounted = True
                    if os.path.exists(img_path):
                        img_mounted = check_mount(img_path)
                    healthy = vm_mounted and img_mounted
 
        if is_active and healthy:
            if svc in ["spark-daemon", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "urbosa", "logos", "mipha", "daruk"]:
...
def check_any_cluster_service_active():
    services = ["spectrum", "aether", "hydra-db", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "urbosa", "logos", "mipha"]
    for svc in services:
        rc, out, _ = run_local(f"systemctl is-active {svc}")
    ...
    if cmd in ["start", "stop", "restart"]:
        if cmd == "stop":
            ...
            if is_all:
                ...
                services = ["logos", "mipha", "spectrum", "bifrost", "dagur", "mimir", "vali", "catalyst", "gatoway", "urbosa", "aether", "hydra-db", "zookeeper", "spark-daemon"]
                for svc in services:
                    rc_act, out_act, _ = run_local(f"systemctl is-active {svc}")
                    if rc_act == 0 and out_act.strip() == "active":
                        print(f"Stopping service {svc}...")
